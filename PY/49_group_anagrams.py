...
# Total Accepted: 99618
# Total Submissions: 323942
# Difficulty: Medium
# Contributors: Admin
...
class Solution(object):
    def groupAnagrams(self, strs):
        ...
        :rtype: List[List[str]]
        """
        mp = {}    # Key is, unique word, value is pos
        l_wp = []   # Word + pos
        pos = 0
#        print(len(strs))
        
        for word in strs:
            tmp = str(sorted(word))     # List is unhashable. 
            if tmp not in mp:
#                print(tmp)
#                print(pos)
                l_wp.append([word, pos])
                mp[tmp] = pos
                pos += 1
            else:
                l_wp.append([word, mp[tmp]])

        # Reconstruct to res
        res = [ [] for x in xrange(pos)]
        for pair in l_wp:
...
if __name__  == "__main__":
    ...
    print(Solution2().groupAnagrams(case))
