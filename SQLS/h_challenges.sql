...
select c.hacker_id, h.name, count(challenge_id) cn
from challenges c
join hackers h on c.hacker_id = h.hacker_id
group by hacker_id
having 
cn = (select count(challenge_id) cn1 from challenges c1 group by c1.hacker_id order by 1 desc limit 1)
or 
cn not in (select count(challenge_id) cn2 from challenges c2 group by c2.hacker_id having c.hacker_id != c2.hacker_id)
